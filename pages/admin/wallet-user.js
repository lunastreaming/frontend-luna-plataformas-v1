...
export default function WalletUserPending() {
  ...
  const fetchPending = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BASE}/api/wallet/admin/pending`, {
        method: 'GET',
        headers,
      ...
      setError('No se pudieron cargar las recargas pendientes')
      setItems([])
    } finally {
      setLoading(false)
    }
  ...
  return (
    <>
      <Head>
        <title>Recargas Pendientes | Admin</title>
      </Head>
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950 text-white">
        ...
        <main className="max-w-6xl mx-auto px-6 py-10">
          <header className="mb-6 flex items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold">Recargas pendientes</h1>
              <p className="text-sm text-gray-400">Revisa y gestiona recargas pendientes de aprobación</p>
            </div>
