...
export default function AddBalanceModal({ open, onClose, onAdd }) {
  ...
  useEffect(() => {
    ...
    // cuando se abre y la moneda es USD, obtener rate
  ...
  const getMinForCurrencyNumber = () => {
    ...
    // round to 2 decimals for both display and validation
    const usdMin = Number(usdMinRaw.toFixed(2))
    return usdMin
  }
